        ...
        <div className="glide__slide layout_cover text-center">
            <div className="mx-auto h-full max-w-xl">
                <div className="no-scrollbar flex h-full w-full flex-col items-center overflow-y-auto overflow-x-hidden p-5 before:m-auto before:content-[''] after:m-auto after:content-[''] ">
                    ...
                    <Link href={`https://calendar.google.com/calendar/event?action=TEMPLATE&dates=${getDateCalendar(dateReception)}00ID/${getDateCalendar(dateReception)}00ID&text=Pernikahan ${item.invitationData ? `${item.invitationData.female.nameShort} dan ${item.invitationData.male.nameShort}` : "Azrina dan Fariz"}&details=Pernikahan ${item.invitationData ? `${item.invitationData.female.nameShort} dan ${item.invitationData.male.nameShort}` : "Azrina dan Fariz"}&location=${item.invitationData ? item.invitationData.reception.address : "Alamat Lengkap Acara"}&recur=RRULE:FREQ%3DWEEKLY;INTERVAL%3D3`} target="_blank" className="animate_animated animate_fadeInUp animate_slower mt-8 rounded-full py-2 px-4 transition-shadow  [animation-delay:calc(var(--animate-duration)*1)] hover:shadow text-sm" rel="nofollow noopener noreferrer" style={{ backgroundColor: item.invitationTheme ? Themes[item.invitationTheme].colorMain : Themes[1].colorMain, color: item.invitationTheme ? Themes[item.invitationTheme].colorBackground : Themes[1].colorBackground }}>Tambahkan Pengingat di Kalender</Link>
                </div>
            </div>
